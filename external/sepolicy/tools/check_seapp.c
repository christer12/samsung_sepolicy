...
key_map rules[] = {
                ...
                { .name = "sdcard",         .type = dt_string,   .dir = dir_out, .data = NULL },
                { .name = "sdcardSuffix",   .type = dt_string,   .dir = dir_out, .data = NULL },
...
static bool rule_map_validate(const rule_map *rm) {
	...
	for(i=0; i < rm->length; i++) {
		...
		if(!strcmp(tmp->name, "user") && tmp->data && !strcmp(tmp->data, "_app")) {
			found_app = true;
		}
	...
	if(found_app && found_name && !found_seinfo) {
